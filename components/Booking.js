...
async function bookHall({ id, jwt ,Date,start,end}) { 
  ...
  const response = await fetch(
    "https://isdl-backend-2-5q8zq3tkb-maanas2403.vercel.app//createBooking?" +
      new URLSearchParams({
        id: id,
        date:Date,
...
async function getDisabled({id,Date}) {
  localDate=Date;
  const response = await fetch("https://isdl-backend-2-5q8zq3tkb-maanas2403.vercel.app/getAllBookings", {
    method: "GET",
  });
